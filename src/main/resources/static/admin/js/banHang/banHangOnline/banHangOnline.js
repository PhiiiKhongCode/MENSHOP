...
$(document).ready(function () {
    $("#buttonDatHang").click(function () {
        ...
        let emailNguoiNhan = $("#nhapEmail").val();
        let tienShipCheck = $("#shippingFee").text();
        let soLuongHienCo = parseInt($("#buttonDatHang").data("soLuongHienCo"));
        ...
        $(".DatHangShipCod .btn-dong-y").click(function () {
            ...
            } else if (soLuongHienCo > 5) {
                Swal.fire({
                    icon: "error",
                    title: "Số lượng sản phẩm vượt quá giới hạn (5)",
                    showConfirmButton: false,
                    timer: 2000,
                });
                console.log("Số lượng sản phẩm vượt quá giới hạn (5)" + response.error);
            }else {
                $.ajax({
